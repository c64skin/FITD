...
#define SC_KSL(mydb) ((uint32) (mydb / (EG_STEP / 2)))
...
void OPLBuildTables(int ENV_BITS_PARAM, int EG_ENT_PARAM) {
	...
	for (i = 0; i < (int)(sizeof(SL_TABLE_SEED) / sizeof(uint32)); i++)
...
INLINE void OPL_CALC_RH(OPL_CH *CH) {
	...
	/* SD */
	if(env_sd < (uint32)(EG_ENT - 1))
		outd[0] += OP_OUT(SLOT7_1, env_sd, 0) * 8;
	/* TAM */
	if(env_tam < (uint32)(EG_ENT - 1))
		outd[0] += OP_OUT(SLOT8_1, env_tam, 0) * 2;
	/* TOP-CY */
	if(env_top < (uint32)(EG_ENT - 1))
		outd[0] += OP_OUT(SLOT7_2, env_top, tone8) * 2;
	/* HH */
	if(env_hh  < (uint32)(EG_ENT-1))
